...
export const fetchCarePlanOptions = createAsyncThunk(
    'carePlans/fetchCarePlanOptions',
    async (clientId, { rejectWithValue }) => {
        try {
            const response = await axios.get(`${API_URL}/clients/${clientId}/care-options`, { withCredentials: true });
            return response.data.data;
        } catch (error) {
            console.error('Error in fetchCarePlanOptions:', error);
...
const carePlansSlice = createSlice({
    ...
    initialState: {
        ...
        carePlanOptionsLoading: false,
        carePlanOptionsError: null,
    },
    reducers: {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(deleteCarePlan.rejected, (state, action) => {
                state.loading = false;
                state.error = action.error.message;
            }).addCase(fetchCarePlanOptions.pending, (state) => {
                state.carePlanOptionsLoading = true;
                state.carePlanOptionsError = null;
            })
            .addCase(fetchCarePlanOptions.fulfilled, (state, action) => {
                state.carePlanOptionsLoading = false;
                state.carePlanOptions = action.payload;
            }).addCase(fetchCarePlanOptions.rejected, (state, action) => {
                state.carePlanOptionsLoading = false;
                state.carePlanOptionsError = action.error.message
            })
            ;
    },
});
